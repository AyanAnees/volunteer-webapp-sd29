...
if exist "C:\Program Files\Git\bin\bash.exe" (
    "C:\Program Files\Git\bin\bash.exe" -c "./fix-hanna-commits.sh"
) else (
    REM Try alternate location
    if exist "C:\Program Files (x86)\Git\bin\bash.exe" (
        "C:\Program Files (x86)\Git\bin\bash.exe" -c "./fix-hanna-commits.sh"
    ) else (
        echo Git Bash not found in standard locations.
        echo Please run the script manually using Git Bash.
        exit /b 1
    )
)
