@echo off
echo Running commit history rewrite with force...
...
if exist "C:\Program Files\Git\bin\bash.exe" (
    "C:\Program Files\Git\bin\bash.exe" -c "./rewrite-history-force.sh"
) else (
    REM Try alternate location
    if exist "C:\Program Files (x86)\Git\bin\bash.exe" (
        "C:\Program Files (x86)\Git\bin\bash.exe" -c "./rewrite-history-force.sh"
    ) else (
        echo Git Bash not found in standard locations.
        echo Please run the rewrite-history-force.sh script manually using Git Bash.
        exit /b 1
    )
)
